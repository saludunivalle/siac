...
const Home = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!response || !Array.isArray(response)) {
          console.error(
            "Error: La respuesta de Filtro5 no es un array válido:",
            response,
          );
          return;
        }
        ...
        response.forEach((item) => {
          ...
          const existingEsActivo =
            existing["estadorc"] === "Vigente" ||
            existing["estadorc"] === "Vigente (En trámite)";
        ...
        console.log(
          `Programas totales: ${response.length}, Programas únicos después de deduplicar: ${datosUnicos.length}`,
        );
        ...
        const activosPorRc = datosUnicos.filter(
          (item) =>
            item["estadorc"] === "Vigente" ||
            item["estadorc"] === "Vigente (En trámite)",
        ).length;
        ...
        console.log(
          "Total activos por RC (detallado):",
          datosUnicos.filter(
            (item) =>
              item["estadorc"] === "Vigente" ||
              item["estadorc"] === "Vigente (En trámite)",
        ...
        const totalActivosV2 = datosUnicos.filter(
          (item) =>
        ...
        const activosPregradoUniversitario = datosUnicos.filter(
          (item) =>
            ...
            (["Pregrado", "Universitario"].includes(
              item["pregrado/posgrado"],
            ) ||
        ...
        const inactivosDesistidoInterno = datosUnicos.filter(
          (item) => item["estado"] === "Desistido Interno",
        ).length;
        const inactivosDesistidoMEN = datosUnicos.filter(
          (item) => item["estado"] === "Desistido MEN",
        ).length;
        const inactivosDesistidoMENSede = datosUnicos.filter(
          (item) => item["estado"] === "Desistido MEN - Sede",
        ).length;
        const inactivosInactivo = datosUnicos.filter(
          (item) => item["estado"] === "Inactivo",
        ).length;
        const inactivosInactivoSede = datosUnicos.filter(
          (item) => item["estado"] === "Inactivo - Sede",
        ).length;
        const inactivosInactivoVencidoRC = datosUnicos.filter(
          (item) => item["estado"] === "Inactivo - Vencido RC",
        ).length;
        const inactivosNegacionRC = datosUnicos.filter(
          (item) => item["estado"] === "Negación RC",
        ).length;
        const sinRegistoRc = datosUnicos.filter(
          (item) => item["estadorc"] === "" || item["estadorc"] === null,
        ).length;
  ...
  const updateCharts = useCallback(
    (data) => {
      ...
      if (selectedNivelAcademico !== "Todos") {
        filteredData = filteredData.filter(
          (item) =>
            item["pregrado/posgrado"] === selectedNivelAcademico &&
            (item["estadorc"] === "Vigente" ||
              item["estadorc"] === "Vigente (En trámite)"),
        );
      }
      ...
      if (selectedNivelFormacion !== "Todos") {
        filteredData = filteredData.filter(
          (item) =>
            item["nivel de formación"] === selectedNivelFormacion &&
            (item["estadorc"] === "Vigente" ||
              item["estadorc"] === "Vigente (En trámite)"),
        );
      }
      ...
      const escuelaCounts = filteredData.reduce((acc, item) => {
        if (
          ...
          if (!acc[escuela]) {
            acc[escuela] = { pregrado: 0, posgrado: 0 };
          }
          if (item["pregrado/posgrado"] === "Pregrado") {
            acc[escuela].pregrado += 1;
          }
          if (item["pregrado/posgrado"] === "Posgrado") {
            acc[escuela].posgrado += 1;
          }
        }
        return acc;
      }, {});

      const escuelasLabels = Object.keys(escuelaCounts);
      const escuelasPregrado = escuelasLabels.map(
        (escuela) => escuelaCounts[escuela]?.pregrado || 0,
      );
      const escuelasPosgrado = escuelasLabels.map(
        (escuela) => escuelaCounts[escuela]?.posgrado || 0,
      );
        ...
        datasets: [
          {
            ...
            borderColor: modernColors.primary.map((color) =>
              color.replace("0.8", "1"),
            ),
            borderWidth: 2,
            hoverBorderWidth: 3,
  ...
  const updateChartsVisibility = useCallback(() => {
    if (
      selectedNivelAcademico === "Todos" &&
      selectedNivelFormacion === "Todos" &&
  ...
  const handleAcademicLevelClick = useCallback(
    (nivelAcademico) => {
  ...
  const handleEscuelaClick = useCallback(
    (escuela) => {
  ...
  const prepareReportData = useCallback(async () => {
    try {
      ...
      const filteredSeguimientos = seguimientos.filter(
        (seg) => seg.usuario === user,
      );

      const reportData = filteredSeguimientos.map((seg) => {
        const programa = programas.find(
          (prog) => prog.id_programa === seg.id_programa,
        );
    ...
    () => ({
      ...
      plugins: {
        ...
        tooltip: {
          ...
          bodyFont: {
            family:
              "'SF Pro Display', -apple-system, BlinkMacSystemFont, 'Segue UI', Roboto, sans-serif",
            weight: "400",
          },
        },
        datalabels: {
              ...
              >
                <Table size={isMobile ? "small" : "medium"}>
                  <TableBody>
                    ...
                    {[
                      {
                        label: "Pregrado Universitario",
                        shortLabel: "P. Universitario",
                        value: programData.activos.pregrado.universitario,
                      },
                      {
                        label: "Pregrado Tecnológico",
                        shortLabel: "P. Tecnológico",
                        value: programData.activos.pregrado.tecnologico,
                      },
                      {
                        label: "Maestría",
                        shortLabel: "Maestría",
                        value: programData.activos.posgrado.maestria,
                      },
                      {
                        label: "Especialización Universitaria",
                        shortLabel: "Esp. Universitaria",
                        value:
                          programData.activos.posgrado
                            .especializacionUniversitaria,
                      },
                      {
                        label: "Especialización Médico Quirúrgica",
                        shortLabel: "Esp. Médico Quir.",
                        value:
                          programData.activos.posgrado
                            .especializacionMedicoQuirurgica,
                      },
                      {
                        label: "Doctorado",
                        shortLabel: "Doctorado",
                        value: programData.activos.posgrado.doctorado,
                      },
                    ].map((item, index) => (
                      <TableRow
                        key={index}
                    ...
                    {[
                      {
                        label: "Desistido Interno",
                        shortLabel: "Desist. Interno",
                        value: programData.inactivos.desistidoInterno,
                      },
                      {
                        label: "Desistido MEN",
                        shortLabel: "Desist. MEN",
                        value: programData.inactivos.desistidoMEN,
                      },
                      {
                        label: "Desistido MEN - Sede",
                        shortLabel: "Desist. MEN-Sede",
                        value: programData.inactivos.desistidoMENSede,
                      },
                      {
                        label: "Inactivo",
                        shortLabel: "Inactivo",
                        value: programData.inactivos.inactivo,
                      },
                      {
                        label: "Inactivo - Sede",
                        shortLabel: "Inactivo-Sede",
                        value: programData.inactivos.inactivoSede,
                      },
                      {
                        label: "Inactivo - Vencido RC",
                        shortLabel: "Inact. Venc. RC",
                        value: programData.inactivos.inactivoVencidoRC,
                      },
                      {
                        label: "Negación RC",
                        shortLabel: "Negación RC",
                        value: programData.inactivos.negacionRC,
                      },
                      {
                        label: "Sin Registro RC",
                        shortLabel: "Sin Reg. RC",
                        value: programData.inactivos.sinRegistroRc,
                      },
                    ].map((item, index) => (
                      <TableRow
                        key={index}
                        ...
                        <Bar
                          ...
                          options={{
                            ...
                            onClick: (event, elements, chart) => {
                              ...
                              const nivelFormacion = chart.data.labels[index];
                          ...
                          options={{
                            ...
                            onClick: (event, elements, chart) => {
                              if (!elements.length || !chart?.data?.labels)
                                return;
                              const { index } = elements[0];
                              const escuela = chart.data.labels[index];
