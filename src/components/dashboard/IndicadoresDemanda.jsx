...
const IndicadoresDemanda = ({
  ...
  const agruparPorPeriodo = (source, cupoKey) => {
    ...
    const conTasa = ordenados.map((row) => ({
      ...row,
      tasa:
        Math.round(calcularTasaInscripcion(row.inscritos, row.cupos) * 100) /
        100,
    }));
  ...
  const construirTabla = (source) => {
    ...
    return Object.values(porPrograma).map((p) => ({
      ...p,
      porcentajeMin:
        Math.round((p.cuposMin ? (p.inscritos / p.cuposMin) * 100 : 0) * 100) /
        100,
      porcentajeMax:
        Math.round((p.cuposMax ? (p.inscritos / p.cuposMax) * 100 : 0) * 100) /
        100,
  ...
  const periodosDisponibles = [
    ...new Set(datos.map((item) => item.periodo)),
  ].sort();
    ...
    <Box>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid item xs={12} lg={6}>
          <ChartCard
            title={`Inscripción y cupo mínimo (con tasa de inscripción)${programaSeleccionado}`}
            data={chartDataFrom(
              datosMin,
              "Cupo mínimo",
              "Tasa inscripción (min)",
            )}
            options={options(
              `Inscripción y cupo mínimo (con tasa de inscripción)${programaSeleccionado}`,
            )}
            type="bar"
            height={400}
          />
        ...
        <Grid item xs={12} lg={6}>
          <ChartCard
            title={`Inscripción y cupo máximo (con tasa de inscripción)${programaSeleccionado}`}
            data={chartDataFrom(
              datosMax,
              "Cupo máximo",
              "Tasa inscripción (max)",
            )}
            options={options(
              `Inscripción y cupo máximo (con tasa de inscripción)${programaSeleccionado}`,
            )}
            type="bar"
            height={400}
          />
