...
const IndicadoresMatricula = ({
  ...
  const estadisticasPorEscuela = useEstadisticasPorEscuela(
    matriculaFilteredData,
  );
  const datosTablaMatricula = useTablaMatriculaData(matriculaFilteredData);
  const { chartData: programasDistribucionData } = useProgramasDistribucion(
    datosTablaMatricula,
    matriculaFilteredData,
  );
  const programaSeleccionado =
    filters.selectedProgramaMatricula !== "Todos"
      ? ` - ${filters.selectedProgramaMatricula}`
      : "";
  ...
  const matriculaChartOptions = {
    ...
    plugins: {
      title: {
        display: true,
        text: `Matriculados por Periodo y Tasas de Crecimiento${programaSeleccionado}`,
      },
      legend: {
        position: "top",
      },
  ...
  const programasDistribucionOptions = {
    ...
    plugins: {
      title: {
        display: true,
        text: `Distribución de Programas por Nivel y Categoría de Crecimiento${programaSeleccionado}`,
      },
      legend: {
        position: "top",
      },
  ...
  const tableColumns = [
    ...
    { header: "TCA Promedio (%)", key: "tcaAnualPromedio", type: "tca" },
    {
      ...
      key: "indicador",
      type: "indicador",
      sourceKey: "tcaAnualPromedio",
    },
  ];

  // Obtener períodos únicos para el filtro
  const periodosDisponibles = [
    ...new Set(matriculaFilteredData.map((item) => item.periodo)),
  ].sort();
    ...
    <Box>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid item xs={12} lg={6}>
          <ChartCard
            title={`Matriculados por Periodo y Tasas de Crecimiento${programaSeleccionado}`}
        ...
        <Grid item xs={12} lg={6}>
          <ChartCard
            title={`Distribución de Programas por Nivel y Categoría de Crecimiento${programaSeleccionado}`}
