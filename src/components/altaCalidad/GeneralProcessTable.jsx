...
const GeneralProcessTable = ({ counts, processConfig, riskConfig, getTotalByProcess, getTotalByRisk, getGrandTotal, handleRowClick }) => {
  return (
        ...
        <Card sx={{ 
          ...
          <CardContent sx={{ p: 0 }}>
            <TableContainer sx={{ overflowX: 'auto', maxWidth: '100%', width: '100%' }}>
              <Table aria-label="tabla de alta calidad" sx={{ tableLayout: { xs: 'auto', md: 'fixed' }, width: '100%', ml: 0 }}>
                <TableHead>
                ...
                <TableBody>
                        ...
                        <Box display="flex" alignItems="center" gap={2}>
                          <Box sx={{
                            ...
                            {processConfig[proceso]?.icon && React.cloneElement(processConfig[proceso].icon, {
                              sx: {
                                color: 'white',
                                fontSize: proceso === 'RAAC' ? '32px' : '28px'
                              }
                            })}
                          </Box>
                          <Box>
                  ...
                  <TableRow sx={{ background: 'linear-gradient(135deg, #F8F9FA 0%, #E9ECEF 100%)', borderTop: '2px solid rgba(0,0,0,0.06)' }}>
                    <TableCell sx={{ py: 3, px: 4, borderBottom: 'none' }}>
                      <Box display="flex" alignItems="center" gap={2}>
                        <EmojiEventsIcon sx={{ color: '#B22222', fontSize: '22px' }} />
