...
const ConsolidadoHistorico = ({ data, programaAcademico, showTitle = true }) => {
  ...
  const processConsolidatedData = (rawData) => {
    ...
    const processedData = rawData
      .filter(item => {
        // Aplicar la misma validación estricta que usamos para los filtros
  ...
  const generarOpcionesFiltros = () => {
    ...
    // Solo usar datos que tienen información histórica válida
  ...
  const calcularEstadisticasGenerales = () => {
    ...
    const programasAcreditadosVigentes = programasArray.filter(p => {
      // Debe tener resolución (columna resolucion de HISTORICO)
      if (!p.resolucion) return false;
      
      // Verificar si la acreditación está vigente
      if (p.fecha_fin) {
        const fechaFin = parseDate(p.fecha_fin);
        if (fechaFin && fechaFin < fechaActual) {
          return false; // Acreditación vencida
        }
      }
          ...
          <Card className="nivel-card card-shadow-hover" sx={{ 
            ...
            <CardContent sx={{ 
              ...
              <Typography variant="caption" sx={{ 
                ...
                {estadisticasGenerales.programasConAcreditacion} con resolución total
              </Typography>
        ...
        <TableContainer component={Paper} sx={{ mt: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
          <Table sx={{ minWidth: 650 }} aria-label="tabla de acreditaciones">
            <TableHead sx={{ backgroundColor: '#f5f5f5' }}>
              <TableRow>
                <TableCell sx={{ fontWeight: 'bold', color: '#2c3e50' }}>Programa Académico</TableCell>
                <TableCell sx={{ fontWeight: 'bold', color: '#2c3e50' }}>Resolución</TableCell>
                ...
                <TableCell sx={{ fontWeight: 'bold', color: '#2c3e50' }}>Duración (años)</TableCell>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Typography variant="body2">
                          {item.isRejected ? 'Rechazada' : 
                           item.isInitial ? 'Acreditación Inicial' : 
