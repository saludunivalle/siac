...
import {
  ...
  Box,
  Backdrop,
} from "@mui/material"; // Añadir Checkbox y Box
...
const Programas = () => {
  ...
  const [interfacultadesCount, setInterfacultadesCount] = useState(0);
  ...
  const interfacultadExclusions = new Set([
    "Facultad de Salud",
    "Salud",
    "#N/A",
  ]);
  const isInterfacultad = (item) => {
    const facultadValue = String(
      item["facultad"] ?? item.facultad ?? "",
    ).trim();
    return facultadValue !== "" && !interfacultadExclusions.has(facultadValue);
  };
    ...
    const fetchDataAndFilter = async () => {
      ...
      try {
        ...
        if (selectedValues.includes("option1")) {
          // Pregrado selected for counts
          programsForCounting = allProgramsGlobal.filter(
            (item) =>
              item["pregrado/posgrado"] === "Pregrado" ||
              item["pregrado/posgrado"] === "Pregrado-Tec" ||
              item["pregrado/posgrado"] == "Universitario",
          );
        } else if (selectedValues.includes("option2")) {
          // Posgrado selected for counts
          programsForCounting = allProgramsGlobal.filter(
        ...
        } else if (isCargo.includes("Posgrados")) {
          // Default for Posgrados role if no specific pre/pos selected
          programsForCounting = allProgramsGlobal.filter(
            (item) => item["pregrado/posgrado"] === "Posgrado",
          );
        }
        ...
        setInterfacultadesCount(
          programsForCounting.filter(
            (item) =>
              (item["estado"] === "Activo" ||
                item["estado"] === "Activo - Sede") &&
              isInterfacultad(item),
          ).length,
        );
        setLoadingState((prev) => ({ ...prev, interfacultades: false }));

        setCreacionCount(
          programsForCounting.filter((item) => item["estado"] === "En Creación")
            .length,
        );
        setLoadingState((prev) => ({ ...prev, creacion: false }));

        setCreacionSedesCount(
          programsForCounting.filter(
            (item) =>
        ...
        const inactiveStatesForCount = [
          ...
          "Desistido",
          "Desistido Interno",
          "Desistido Interno - Sede",
          "Desistido MEN",
          "Desistido MEN - Sede",
          "Rechazado",
        ];
        setInactivosCount(
          programsForCounting.filter((item) =>
            inactiveStatesForCount.includes(item["estado"]),
          ).length,
        );
        ...
        setVencidoRCCount(
          programsForCounting.filter(
            (item) => item["estado"] === "Inactivo - Vencido RC",
          ).length,
        );
        setLoadingState((prev) => ({ ...prev, vencidoRC: false }));

        setDesistidoMenCount(
          programsForCounting.filter(
            (item) =>
        ...
        setRechazadoCount(
          programsForCounting.filter((item) =>
            ["Rechazado", "Negación RC", "Negación AAC"].includes(
              item["estado"],
            ),
          ).length,
        );
        ...
        setDoctoradoCount(
          posgradoActivos.filter(
            (item) => item["nivel de formación"] === "Doctorado",
          ).length,
        );
        ...
        setMaestriaCount(
          posgradoActivos.filter(
            (item) => item["nivel de formación"] === "Maestría",
          ).length,
        );
        ...
        if (
          isCargo.includes("Posgrados") &&
          !selectedValues.includes("option1") &&
          ...
          result = result.filter((item) => {
            return selectedValues.some((option) => {
              switch (option) {
                ...
                case "option4":
                  return item["estado"] === "En Creación"; // Creacion Cali
                case "option8":
                  return (
                    item["estado"] === "En Creación*" ||
                    item["estado"] === "En Creación - Sede"
                  ); // Creacion Sedes/Otros
                case "inactive":
                  return (
                    item["estado"] === "Inactivo" ||
                    item["estado"] === "Inactivo - Sede"
                ...
                case "vencido-rc":
                  return item["estado"] === "Inactivo - Vencido RC";
                case "desistido":
                  return (
  ...
  const getCellBackgroundColor = (item, process) => {
    const seguimientos = filteredDataSeg.filter(
      (seg) => seg.id_programa === item.id_programa,
    );
    const defaultGreyCondition =
      (process === "CREA" &&
        (item["estado"] === "En Creación" ||
          item["estado"] === "En Creación*")) ||
    ...
    const seguimientosPorProceso = seguimientos.filter(
      (seg) => seg.topic === topicMap[process],
    );
    ...
    setSelectedValues((prevSelectedValues) => {
      ...
      if (buttonValue === "option1" || buttonValue === "option2") {
        ...
        if (!newSelectedValues.includes("option3"))
          newSelectedValues.push("option3");
        if (!newSelectedValues.includes("option7"))
          newSelectedValues.push("option7");
      }
      // Para Inactivos (option6) - mantener lógica actual
      else if (buttonValue === "option6") {
        const inactiveSubButtons = [
          "inactive",
          "desistido",
          "desistido-int",
          "rechazado",
        ...
        if (!newSelectedValues.includes(buttonValue)) {
          ...
          newSelectedValues = newSelectedValues.filter(
            (val) => !inactiveSubButtons.includes(val),
          );
        }
      ...
      let filteredResult = rowData.filter((item) => {
        ...
        return (
          pregradoOrPosgradoFilter &&
          (nivelesFormacionFilter ||
            !newSelectedValues.some((val) =>
              [
                "doctorado",
                "especializacion",
                "especializacionMedico",
    ...
    <>
      ...
      <div className="table-buttons">
        ...
        >
          <div style={{ fontSize: "18px" }}>
            <strong>Total Programas: </strong> {filteredData.length}{" "}
          </div>
          <ButtonsContainer>
            ...
            <ButtonRow>
              ...
              >
                <Checkbox
                  ...
                  style={{
                    color: isButtonSelected("option1") ? "white" : "grey",
                    padding: "0 5px 0 0",
                  }}
                  size="small"
                />
              ...
              >
                <Checkbox
                  ...
                  style={{
                    color: isButtonSelected("option2") ? "white" : "grey",
                    padding: "0 5px 0 0",
                  }}
                  size="small"
                />
            ...
            {selectedValues.includes("option2") && (
              <ButtonRow>
                ...
                <Button
                  value="especializacionMedico"
                  className="custom-radio2"
                  style={setButtonStyles("especializacionMedico")}
                  onClick={() => handleButtonClick("especializacionMedico")}
                >
                  Especialización Médico Quirúrgica <br />{" "}
                  {renderCount(espMedicoCount, "espMedico")}
                </Button>
                <Button
                  value="especializacionUni"
                  className="custom-radio2"
                  style={setButtonStyles("especializacionUni")}
                  onClick={() => handleButtonClick("especializacionUni")}
                >
                  Especialización Universitaria <br />{" "}
                  {renderCount(espUniCount, "espUni")}
                </Button>
                <Button
                  value="maestria"
                  className="custom-radio2"
            ...
            <ButtonRow>
              ...
              >
                Activos Cali
                <br /> {renderCount(activosCount, "activos")}
              </Button>
              <Button
                value="option7"
                className="custom-radio2"
              ...
              >
                Activos Sedes <br />{" "}
                {renderCount(activoSedesCount, "activosSede")}
              </Button>
              <Button
                value="interfacultades"
                className="custom-radio2"
                style={setButtonStyles("interfacultades")}
                onClick={() => handleButtonClick("interfacultades")}
              >
                Activos Interfacultades <br />{" "}
                {renderCount(interfacultadesCount, "interfacultades")}
              </Button>
              <Button
                value="option4"
                className="custom-radio2"
              ...
              >
                Creación Cali
                <br /> {renderCount(creacionCount, "creacion")}
              </Button>
              <Button
                value="option8"
                className="custom-radio2"
              ...
              >
                Creación (Sedes y otros) <br />{" "}
                {renderCount(creacionSedesCount, "creacionSedes")}
              </Button>
              <Button
                value="option6"
                className="custom-radio2"
            ...
            {showInactiveButtons && (
              <ButtonRow>
                ...
                >
                  {" "}
                  Inactivo <br /> {renderCount(inactiveCount, "inactive")}
                </Button>
                <Button
                  value="vencido-rc"
                  className="custom-radio2"
                  style={setButtonStyles("vencido-rc")}
                  onClick={() => handleButtonClick("vencido-rc")}
                >
                  {" "}
                  Vencido RC <br /> {renderCount(vencidoRCCount, "vencidoRC")}
                </Button>
                <Button
                  value="desistido"
                  className="custom-radio2"
                  style={setButtonStyles("desistido")}
                  onClick={() => handleButtonClick("desistido")}
                >
                  {" "}
                  Desistido MEN <br />{" "}
                  {renderCount(desistidoMenCount, "desistidoMen")}
                </Button>
                <Button
                  value="desistido-int"
                  className="custom-radio2"
                  style={setButtonStyles("desistido-int")}
                  onClick={() => handleButtonClick("desistido-int")}
                >
                  {" "}
                  Desistido Interno <br />{" "}
                  {renderCount(desistidoInternoCount, "desistidoInterno")}
                </Button>
                <Button
                  value="rechazado"
                  className="custom-radio2"
                  style={setButtonStyles("rechazado")}
                  onClick={() => handleButtonClick("rechazado")}
                >
                  {" "}
                  Rechazado <br /> {renderCount(rechazadoCount, "rechazado")}
                </Button>
              </ButtonRow>
            )}
          </ButtonsContainer>
      ...
      {filteredData && filteredData.length > 0 ? (
        <div className="row-container">
          ...
          >
            {filteredData.some(
              (data) => data["escuela"] === "Bacteriología y Lab. Clínico",
            ) && (
              <CollapsibleButton
                defaultClosed={
                  escuelaDirector !== "Bacteriología y Lab. Clínico"
                }
                buttonText={`Bacteriología y Lab. Clínico (${Filtro4(filteredData, "Bacteriología y Lab. Clínico").length})`}
                content={renderFilteredTable(
                  filteredData,
            ...
            {filteredData.some(
              (data) => data["escuela"] === "Ciencias Básicas",
            ) && (
              <CollapsibleButton
                defaultClosed={escuelaDirector !== "Ciencias Básicas"}
                buttonText={`Ciencias Básicas (${Filtro4(filteredData, "Ciencias Básicas").length})`}
                content={renderFilteredTable(filteredData, "Ciencias Básicas")}
              />
            )}
            {filteredData.some((data) => data["escuela"] === "Enfermería") && (
              <CollapsibleButton
                defaultClosed={escuelaDirector !== "Enfermería"}
                buttonText={`Enfermería (${Filtro4(filteredData, "Enfermería").length})`}
                content={renderFilteredTable(filteredData, "Enfermería")}
              />
            )}

            {filteredData.some((data) => data["escuela"] === "Odontología") && (
              <CollapsibleButton
                defaultClosed={escuelaDirector !== "Odontología"}
                buttonText={`Odontología (${Filtro4(filteredData, "Odontología").length})`}
                content={renderFilteredTable(filteredData, "Odontología")}
              />
            )}
            {filteredData.some(
              (data) => data["escuela"] === "Rehabilitación Humana",
            ) && (
              <CollapsibleButton
                defaultClosed={escuelaDirector !== "Rehabilitación Humana"}
                buttonText={`Rehabilitación Humana (${Filtro4(filteredData, "Rehabilitación Humana").length})`}
                content={renderFilteredTable(
                  filteredData,
            ...
            {filteredData.some(
              (data) => data["escuela"] === "Salud Pública",
            ) && (
              <CollapsibleButton
                defaultClosed={escuelaDirector !== "Salud Pública"}
                buttonText={`Salud Pública (${Filtro4(filteredData, "Salud Pública").length})`}
                content={renderFilteredTable(filteredData, "Salud Pública")}
              />
            )}
            {filteredData.some(
              (data) => data["escuela"] === "Dirección de Posgrados",
            ) && (
              <CollapsibleButton
                defaultClosed={escuelaDirector !== "Dirección de Posgrados"}
                buttonText={`Dirección de Posgrados (${Filtro4(filteredData, "Dirección de Posgrados").length})`}
                content={renderFilteredTable(
                  filteredData,
            ...
            {filteredData.some((data) => data["escuela"] === "Medicina") && (
              <CollapsibleButton
                defaultClosed={escuelaDirector !== "Medicina"}
                buttonText={`Medicina (${Filtro4(filteredData, "Medicina").length})`}
                content={renderFilteredTable(filteredData, "Medicina")}
              />
            )}
            {filteredData.some(
              (item) =>
