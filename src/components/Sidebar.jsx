...
const Sidebar = ({ isCargo }) => {
  ...
  const hasPermission = (permission) =>
    ...
    isCargo.some((cargo) => {
      const normalizedCargo = String(cargo || "")
        .trim()
        .toLowerCase();
    ...
    if (sessionStorage.getItem("logged")) {
      ...
      if (res.length > 0) {
        ...
        const usuarioExtraido =
          res[0]?.user || res[0]?.usuario || res[0]?.email || "";
  ...
  const prepareReportData = async () => {
    try {
      ...
      console.log(
        "Seguimientos filtrados por usuario:",
        filteredSeguimientos.length,
      );

      if (filteredSeguimientos.length === 0) {
        console.warn("No se encontraron seguimientos para el usuario:", user);
        console.log("Usuarios únicos en seguimientos:", [
          ...new Set(seguimientos.map((seg) => seg.usuario)),
        ]);
      }
          ...
          sx={{
            ...
            backgroundColor: active
              ...
              : hoveredItem === itemKey
                ...
                : "transparent",
            transform:
              hoveredItem === itemKey ? "translateX(4px)" : "translateX(0px)",
            boxShadow: active
  ...
  return (
    <>
          ...
          >
            ...
            >
              <ListItem
                ...
                onClick={() =>
                  openExternalLink(
                    "https://lookerstudio.google.com/u/1/reporting/9e58572e-acd8-411a-8635-3767e4729091/page/p_9zoaa6qptd",
                  )
                }
      ...
      {(isHidden || (isMobile && !open)) && (
        ...
        >
          <Tooltip title="Mostrar menú" placement="right" arrow>
            <IconButton
              onClick={() => {
                setIsHidden(false);
                setOpen(true);
