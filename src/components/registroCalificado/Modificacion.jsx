...
const parseDate = (value) => {
  ...
  if (parts.length === 3) {
    ...
    if ([day, month, year].every(Number.isFinite)) return new Date(year, month - 1, day);
  }
...
const Modificacion = ({
  ...
  const getUniqueOptions = (field, source = procesoProgramas) => [
    ...new Set(source.map((p) => normalize(p[field])).filter(Boolean)),
  ].sort();

  const visibleBase = useMemo(() => {
    if (selectedSubprocess === "sustanciales") {
      return procesoProgramas.filter((program) => normalize(program.mod_sus).toUpperCase() === "SI");
    }
    if (selectedSubprocess === "noSustanciales") {
      return procesoProgramas.filter((program) => normalize(program.mod_sus).toUpperCase() === "NO");
    }
  ...
  const filteredPrograms = useMemo(() => {
    return visibleBase.filter((program) => {
      const programaAcademico = normalize(getFieldValue(program, "programa académico", "programa academico"));
      const nivelAcademico = normalize(getFieldValue(program, "pregrado/posgrado", "nivel academico"));
      const nivelFormacion = normalize(getFieldValue(program, "nivel de formación", "nivel de formacion"));

      if (filters.escuela && normalize(program.escuela) !== normalize(filters.escuela)) return false;
      if (filters.programa && programaAcademico !== normalize(filters.programa)) return false;
      if (filters.nivelAcademico && nivelAcademico !== normalize(filters.nivelAcademico)) return false;
      if (filters.nivelFormacion && nivelFormacion !== normalize(filters.nivelFormacion)) return false;
      return true;
    });
  }, [visibleBase, filters]);
  ...
  const rows = useMemo(() => {
    return filteredPrograms.map((program) => {
      const latestMod = getLatestSeguimientoForProcess(seguimientos, program.id_programa, "MOD");
      return {
        ...program,
        programaAcademico: normalize(getFieldValue(program, "programa académico", "programa academico")),
        nivelAcademico: normalize(getFieldValue(program, "pregrado/posgrado", "nivel academico")),
        nivelFormacion: normalize(getFieldValue(program, "nivel de formación", "nivel de formacion")),
  ...
  return (
    <Card sx={{ boxShadow: "0 1px 3px rgba(0,0,0,0.02), 0 8px 24px rgba(0,0,0,0.04)", borderRadius: "20px", overflow: "hidden", border: "1px solid rgba(0,0,0,0.02)", width: "100%" }}>
      <Box sx={{ p: { xs: 2, sm: 3 }, background: "linear-gradient(135deg, #FAFBFC 0%, #FFFFFF 100%)", borderBottom: "1px solid rgba(0,0,0,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
      ...
      <Box sx={{ px: 3, pt: 2, pb: 1, display: "flex", gap: 2, flexWrap: "wrap", alignItems: "center" }}>
        <Button variant={selectedSubprocess === "all" ? "contained" : "outlined"} onClick={() => setSelectedSubprocess("all")} sx={{ borderColor: "#B22222", color: selectedSubprocess === "all" ? "white" : "#B22222", backgroundColor: selectedSubprocess === "all" ? "#B22222" : "transparent" }}>
          Todas
        </Button>
        <Button variant={selectedSubprocess === "sustanciales" ? "contained" : "outlined"} onClick={() => setSelectedSubprocess("sustanciales")} sx={{ borderColor: "#B22222", color: selectedSubprocess === "sustanciales" ? "white" : "#B22222", backgroundColor: selectedSubprocess === "sustanciales" ? "#B22222" : "transparent" }}>
          Sustanciales
        </Button>
        <Button variant={selectedSubprocess === "noSustanciales" ? "contained" : "outlined"} onClick={() => setSelectedSubprocess("noSustanciales")} sx={{ borderColor: "#B22222", color: selectedSubprocess === "noSustanciales" ? "white" : "#B22222", backgroundColor: selectedSubprocess === "noSustanciales" ? "#B22222" : "transparent" }}>
          No sustanciales
        </Button>
      </Box>
      ...
      <Card sx={{ mb: 3, borderRadius: 4, border: "1px solid rgba(0,0,0,0.04)" }}>
        <CardContent sx={{ display: "grid", gap: 2, gridTemplateColumns: { xs: "1fr", md: "repeat(4, 1fr)" } }}>
          <FormControl fullWidth size="small">
            <InputLabel>Escuela</InputLabel>
            <Select
              value={filters.escuela}
              label="Escuela"
              onChange={(e) => setFilters((prev) => ({ ...prev, escuela: e.target.value, programa: "" }))}
            >
          ...
          <FormControl fullWidth size="small">
            ...
            >
              ...
              {getUniqueOptions("programa académico", filters.escuela ? visibleBase.filter((p) => normalize(p.escuela) === normalize(filters.escuela)) : visibleBase).map((option) => (
                <MenuItem key={option} value={option}>{option}</MenuItem>
              ))}
            </Select>
          </FormControl>
          <FormControl fullWidth size="small">
            ...
            <Select
              ...
              onChange={(e) => setFilters((prev) => ({ ...prev, nivelAcademico: e.target.value }))}
            >
              <MenuItem value="">Todos</MenuItem>
              {getUniqueOptions("pregrado/posgrado", visibleBase).map((option) => (
                <MenuItem key={option} value={option}>{option}</MenuItem>
              ))}
            </Select>
          </FormControl>
          <FormControl fullWidth size="small">
            ...
            <Select
              ...
              onChange={(e) => setFilters((prev) => ({ ...prev, nivelFormacion: e.target.value }))}
            >
              <MenuItem value="">Todos</MenuItem>
              {getUniqueOptions("nivel de formación", visibleBase).map((option) => (
                <MenuItem key={option} value={option}>{option}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </CardContent>
        <Box sx={{ px: 2, pb: 2, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
          <Typography variant="body2" sx={{ color: "#070707", fontWeight: 600 }}>
            {sortedRows.length} programa{sortedRows.length === 1 ? "" : "s"} encontrado{sortedRows.length === 1 ? "" : "s"}
          </Typography>
          <Button onClick={clearFilters} variant="text" sx={{ color: "#B22222" }}>
            Limpiar filtros
          </Button>
        </Box>
      </Card>
                ...
                {[
                  ...
                  >
                    <TableSortLabel
                      ...
                      sx={{
                        "& .MuiTableSortLabel-icon": { color: orderBy === column.key ? "#B22222" : "inherit" },
                        "&.Mui-active": {
                          color: "#B22222",
