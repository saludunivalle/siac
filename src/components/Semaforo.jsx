...
const Semaforo = ({ globalVariable }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setWhiteProgramsCount(
          response.filter(
            (item) =>
              item["fase rrc"] === "Vencido" &&
              item["rc vigente"] === "SI" &&
              item["sede"] === "Cali",
          ).length,
        );
        setGreenProgramsCount(
          response.filter(
            (item) =>
              item["fase rrc"] === "Fase 1" &&
              item["rc vigente"] === "SI" &&
              item["sede"] === "Cali",
          ).length,
        );
        setYellowProgramsCount(
          response.filter(
            (item) =>
              item["fase rrc"] === "Fase 2" &&
              item["rc vigente"] === "SI" &&
              item["sede"] === "Cali",
          ).length,
        );
        setOrangeProgramsCount(
          response.filter(
            (item) =>
              item["fase rrc"] === "Fase 3" &&
              item["rc vigente"] === "SI" &&
              item["sede"] === "Cali",
          ).length,
        );
        setOrange2ProgramsCount(
          response.filter(
            (item) =>
              item["fase rrc"] === "Fase 4" &&
              item["rc vigente"] === "SI" &&
              item["sede"] === "Cali",
          ).length,
        );
  ...
  const handleButtonClick = async (buttonType) => {
    ...
    setHeaderBackgroundColor(
      headerColorMap[buttonType] || headerColorMap.default,
    );
              ...
              {filteredData.map((item, index) => {
                ...
                return (
                  <tr key={index} onClick={() => handleRowClick(item)}>
                    ...
                    <td className="bold" style={{ backgroundColor: rrcColor }}>
                      {item["fechavencrc"]}
                    </td>
                    <td>
                      {item["estadoac"] === "Vigente" ||
                      item["estadoac"] === "Vigente (En trámite)" ? (
                        <img
                          src={Si_icon}
                          alt=""
                          style={{ width: "25px", height: "25px" }}
