...
import {
  ...
  Filtro7,
  FiltroHistorico,
  FiltroHistoricoTimeline,
  getSeguimientoPMByPrograma,
} from "../service/data";
...
const ProgramDetails = () => {
  ...
  // Director Escuela
  const puedesVerTodoEscuela =
    esDirectorEscuela && escuelaPrograma === userEscuela;
  const puedesVerSoloBasicoEscuela =
    esDirectorEscuela && escuelaPrograma !== userEscuela;

  // Director Programa
  const puedesVerTodoPrograma =
    esDirectorPrograma && nombrePrograma === userPrograma;
  const puedesVerSoloBasicoPrograma =
    esDirectorPrograma && nombrePrograma !== userPrograma;
  ...
  const seguimientosCreacion = Array.isArray(filteredDataSeg)
    ? filteredDataSeg.filter(
        (seg) =>
          seg.id_programa ===
            (rowData.id_programa || rowData.id || rowData.ID) &&
          seg.topic === "Creación",
      )
    : [];
  ...
  useEffect(() => {
    ...
    const found = fasesProgramas.find(
      (f) =>
        String(f.id_programa) === String(idProg) && String(f.id_fase) === "28",
    );
      ...
      try {
        ...
        const uniqueOptions = {
          ...
          "Nivel Académico": [
            ...new Set(
              data
                .map((item) => item["pregrado/posgrado"])
                .filter((option) => option && option.trim() !== " "),
            ),
          ],
          "Nivel de Formación": [
            ...new Set(
              data
                .map((item) => item["nivel de formación"])
                .filter((option) => option && option.trim() !== " "),
            ),
          ],
          Jornada: [
  ...
  const [formData, setFormData] = useState({
    ...
    "Número renovaciones RRC":
      normalizeValue(rowData["número renovaciones RRC"]) || 1,
  ...
  const showSeguimientoCreacion =
    isUserLoggedIn && (seguimientosCreacion.length > 0 || sniesEnCreacion);
    ...
    const fetchFiltroHistorico = async () => {
      try {
        ...
        const filteredHistorial = historial.filter(
          (item) => item.id_programa === rowData.id_programa,
        );
  ...
  useEffect(() => {
    const fetchTimelineData = async () => {
      try {
        ...
        const filteredTimelineData = historicoData.filter(
          (item) => item.id_programa === rowData.id_programa,
        );
  ...
  const getSeguimientoBackgroundColor = (process, isSelected) => {
    ...
    const seguimientos = Array.isArray(filteredDataSeg)
      ? filteredDataSeg.filter((seg) => seg.id_programa === rowData.id_programa)
      : [];
    ...
    console.log(
      `Seguimientos para proceso ${process}:`,
      seguimientosPorProceso,
    );
      ...
      if (response.data.status) {
        ...
        try {
          ...
          const updatedProgram = updatedPrograms.find(
            (p) => p.id_programa === rowData.id_programa,
          );
          ...
          if (updatedProgram) {
            ...
            setFormData({
              ...
              "Nivel Académico": normalizeValue(
                updatedProgram["pregrado/posgrado"],
              ),
              "Nivel de Formación": normalizeValue(
                updatedProgram["nivel de formación"],
              ),
              "Titulo a Conceder": normalizeValue(
                updatedProgram["titulo a conceder"],
              ),
              ...
              "Número renovaciones RRC":
                normalizeValue(updatedProgram["número renovaciones RRC"]) || 1,
            });
          }
          ...
          const seguimientoPM = await getSeguimientoPMByPrograma(
            rowData.id_programa,
          );
  ...
  const tabSx = (process) => ({
    backgroundColor: getSeguimientoBackgroundColor(
      process,
  ...
  const exportSeguimientoTabsToExcel = () => {
    ...
    let hasData = false;
    tabs.forEach((tab) => {
      const data = filteredDataSeg.filter(
        (seg) => seg.id_programa === idPrograma && seg.topic === tab.topic,
      );
            ...
            <div className="about-program-general">
              ...
              <div className="about-program-section">
                <div className="about-program">
                  ...
                  {formData["Facultad"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Escuela: </strong>&nbsp;{" "}
                  {formData["Escuela"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Departamento: </strong>&nbsp;{" "}
                  {formData["Departamento"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>SNIES: </strong>&nbsp; {formData["SNIES"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Sección: </strong>&nbsp;{" "}
                  {formData["Sección"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Sede: </strong>&nbsp; {formData["Sede"] || "N/A"}
                </div>
              </div>
              ...
              <div className="about-program-section">
                <div className="about-program">
                  ...
                  {formData["Nivel Académico"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Nivel de Formación: </strong>&nbsp;{" "}
                  {formData["Nivel de Formación"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Jornada: </strong>&nbsp;{" "}
                  {formData["Jornada"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Modalidad: </strong>&nbsp;{" "}
                  {formData["Modalidad"] || "N/A"}
                </div>
              </div>
              ...
              <div className="about-program-section">
                ...
                <div className="about-program">
                  ...
                  {formData["Créditos"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Periodicidad: </strong>&nbsp;{" "}
                  {formData["Periodicidad"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Duración: </strong>&nbsp;{" "}
                  {formData["Duración"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Proceso de Contingencia: </strong>&nbsp;{" "}
                  {formData["Contingencia"] || "N/A"}
                </div>
              </div>
              ...
              <div className="about-program-section">
                <div className="about-program">
                  <strong>Fecha Otorgamiento RRC: </strong>&nbsp;{" "}
                  {formData["FechaExp RRC"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Fecha Vencimiento RRC: </strong>&nbsp;{" "}
                  {formData["Fecha RRC"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Resolución RRC: </strong>&nbsp;{" "}
                  <span
                    dangerouslySetInnerHTML={{
                      __html: documentLinks.rrc || "N/A",
                    }}
                  />
                </div>
                <div className="about-program">
                  <strong>Número Renovaciones RRC: </strong>&nbsp;{" "}
                  {formData["Número renovaciones RRC"] || "N/A"}
                </div>
              </div>
              ...
              <div className="about-program-section">
                <div className="about-program">
                  <strong>Fecha Otorgamiento RAAC: </strong>&nbsp;{" "}
                  {formData["FechaExp RAAC"] || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Fecha Vencimiento RAAC: </strong>&nbsp;{" "}
                  {formData["Fecha RAAC"] || "N/A"}
                </div>
              ...
              <div className="about-program-section">
                <div className="about-program">
                  <strong>
                    Estado del plan de mejoramiento en Registro Calificado:{" "}
                  </strong>
                  &nbsp; {seguimientoPMData?.estado_rc || "N/A"}
                </div>
                <div className="about-program">
                  <strong>
                    Estado del plan de mejoramiento en Acreditación:{" "}
                  </strong>
                  &nbsp; {seguimientoPMData?.estado_ac || "N/A"}
                </div>
                <div className="about-program">
                  <strong>Documentos del programa: </strong>&nbsp;{" "}
                  {(
                    <a
            ...
            {!isUserLoggedIn && (
              ...
              >
                ...
                <Typography variant="body1" sx={{ color: "#666" }}>
                  Para ver seguimientos, editar información o realizar otras
                  acciones, por favor inicia sesión.
                </Typography>
              </Box>
            )}
          </>
                      ...
                      >
                        <Grid item xs={6}>
                          <DatePicker
                            label={
                              key === "FechaExp RRC"
                                ? "Fecha Otorgamiento RRC"
                                : key === "FechaExp RAAC"
                                  ? "Fecha Otorgamiento RAAC"
                                  : key
                            }
                            value={
                              formData[key]
                        ...
                        <Grid item xs={6}>
                          <DatePicker
                            label={
                              key === "FechaExp RRC"
                                ? "Fecha Vencimiento RRC"
                                : "Fecha Vencimiento RAAC"
                            }
                            value={
                              formData[
                                key === "FechaExp RRC"
        ...
        {!puedesVerSoloBasico && (
          <>
            ...
            isValidValue(fase28Data.fecha) ? (
              <div
                className="about-program-section"
                style={{
                  display: "flex",
            ...
            {clickedButton === "crea" ? (
              !showSeguimientoCreacion ? (
                formData["Fecha registro SNIES"] &&
                formData["Fecha registro SNIES"] !== "N/A" &&
                formData["Enlace resolución"] &&
                formData["Enlace resolución"] !== "N/A" ? (
                  <div
                    className="about-program-section"
                    style={{
                      display: "flex",
                  ...
                  >
                    <div className="about-program">
                      Creado el {formData["Fecha registro SNIES"] || "N/A"} con
                      la resolución&nbsp;
                      <span>
                        {formData["Enlace resolución"] ? (
                          <a
                ...
                <Seguimiento
                  ...
                  rowData={{
                    ...rowData,
                    id_programa:
                      rowData.id_programa || rowData.id || rowData.ID || "N/A",
                  }}
                />
              )
