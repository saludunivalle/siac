...
const filterByProperty = async (sheetName, data, propertyName) => {
  try {
    ...
    if (!response.data || !Array.isArray(response.data)) {
      console.error(
        "Error: La respuesta no contiene un array válido en data:",
        response,
      );
      return [];
    }
    ...
    if (data) {
      ...
      if (searchTerm) {
        const filteredData = response.data.filter((item) => {
          ...
          return (
            propiedadValue &&
            propiedadValue.toLowerCase().includes(searchTerm.toLowerCase())
          );
        });
        return filteredData;
      }
...
export const Filtro1 = (data) =>
  filterByProperty(hojaProgramas, data, "pregrado/posgrado");
export const Filtro2 = (data) =>
  filterByProperty(hojaProgramas, data, "fase rrc");
export const Filtro3 = (data) =>
  filterByProperty(hojaProgramas, data, "nivel de formación");
export const Filtro4 = (datos, termino_a_filtrar) => {
  try {
    ...
    const filteredData = datos.filter((item) => {
      ...
      return (
        propiedadValue &&
        propiedadValue.toLowerCase().includes(termino_a_filtrar.toLowerCase())
      );
    });
...
export const Filtro6 = (data) =>
  filterByProperty(hojaProgramas, data, "fase rac");
...
export const Filtro8 = (datos, terminos_a_filtrar) => {
  try {
    ...
    if (
      !Array.isArray(terminos_a_filtrar) ||
      terminos_a_filtrar.some((term) => typeof term !== "string")
    ) {
    ...
    const terminosNormalizados = terminos_a_filtrar.map((t) =>
      t.toLowerCase().trim(),
    );

    // Mostrar topics únicos en los datos
    const topicsExistentes = [
      ...new Set(datos.map((d) => d.topic).filter(Boolean)),
    ];
...
export const Filtro9 = (datos, termino_a_filtrar) => {
  try {
    if (!Array.isArray(datos)) {
      console.warn(
        "Filtro9: Los datos proporcionados no son un array, retornando array vacío",
      );
      return [];
    }
    ...
    if (
      ...
      console.warn(
        "Filtro9: El término de filtrado no es válido:",
        termino_a_filtrar,
        "retornando array vacío",
      );
      return [];
    }
    ...
    const filteredData = datos.filter((item) => {
      ...
      return (
        ...
        propiedadValue
          .toString()
          .toLowerCase()
          .includes(termino_a_filtrar.toLowerCase())
      );
    });
      ...
      "https://siac-server.vercel.app/updateSeguimientoRow",
      {
        ...
        body: JSON.stringify({
          ...
          updateData: [
            ...
            data.estado_act || "",
            data.fase_grande_ok || "",
            data.id_proc_historico || "",
            data.verificado ?? "",
            data.verificado_por || "",
            data.fecha_verificado || "",
          ],
        }),
      },
    );
...
export const deleteSeguimiento = async (data) => {
  try {
    const response = await fetch(
      "https://siac-server.vercel.app/deleteSeguimientoRow",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const obtenerFasesProceso = async () => {
  try {
    ...
    if (!hoja1Response || hoja1Response.status === false) {
      console.error(
        "Error del servidor en obtenerFasesProceso:",
        hoja1Response?.error || "Error desconocido",
      );
      return [];
    }
...
export const Filtro13 = async () => {
  try {
    ...
    console.log(
      `Filtro13: Datos obtenidos exitosamente, ${response.data.length} registros`,
    );
    return response.data;
  } catch (error) {
    ...
    if (error.name === "TypeError" && error.message.includes("fetch")) {
      console.error(
        "Error de red detectado - verificar conectividad y URL del servidor",
      );
    }
...
export const Filtro14 = async () => {
  try {
    ...
    console.log(
      `Filtro14: Datos obtenidos exitosamente, ${response.data.length} registros`,
    );
    return response.data;
  } catch (error) {
...
export const Filtro15 = async () => {
  try {
    ...
    console.log(
      `Filtro15: Datos obtenidos exitosamente, ${response.data.length} registros`,
    );
    return response.data;
  } catch (error) {
...
export const Filtro16 = async () => {
  try {
    ...
    console.log(
      `Filtro16: Datos obtenidos exitosamente, ${response.data.length} registros`,
    );
    return response.data;
  } catch (error) {
...
export const FiltroFirmas = async () => {
  try {
    ...
    if (!response || response.status === false) {
      console.error(
        "Error del servidor en FiltroFirmas:",
        response?.error || "Error desconocido",
      );
      return [];
    }
...
export const dataSegui = async () => {
  try {
    ...
    if (!response || response.status === false) {
      console.error(
        "Error del servidor en dataSegui:",
        response?.error || "Error desconocido",
      );
      return [];
    }
...
export const dataEscuelas = async () => {
  try {
    ...
    if (!response || response.status === false) {
      console.error(
        "Error del servidor en dataEscuelas:",
        response?.error || "Error desconocido",
      );
      return [];
    }
...
export const dataProgramas = async () => {
  try {
    ...
    if (!response || response.status === false) {
      console.error(
        "Error del servidor en dataProgramas:",
        response?.error || "Error desconocido",
      );
      return [];
    }
...
export const FiltroHistoricoTimeline = () =>
  filterByProperty(hojaHistorico, {}, "");
